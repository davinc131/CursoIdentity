...
namespace WebApp.Identity
{
  public class MyUserStore : IUserStore<MyUser>, IUserPasswordStore<MyUser>
  {
    ...
    {
      await SetPasswordHashAsync(user, user.PasswordHash, cancellationToken);
    ...
    public async Task<IdentityResult> CreateAsync(MyUser user, string password, CancellationToken cancellationToken)
    {
      using (var connection = GetOpenConnection())
      {
        await connection.ExecuteAsync(
    ...
    {
      var connection = new SqlConnection("Persist Security Info=False;User ID=sa;Password=sipef@adm;Initial Catalog=IdentityCurso;Data Source=DESKTOP-6MGSSHL");
      connection.Open();
      return connection;
    }
    ...
    public async Task<MyUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken)
    {
      using (var connection = GetOpenConnection())
      {
        return await connection.QueryFirstOrDefaultAsync<MyUser>("select * from Users where UserName = @userName", new { userName = normalizedUserName });
      }
    }
